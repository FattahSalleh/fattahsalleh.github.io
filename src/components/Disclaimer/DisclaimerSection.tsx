        ...
        <div className="h-auto w-auto flex flex-row flex-no-wrap items-center justify-center">
          <ul className="flex flex-row flex-no-wrap">
            {disclaimerData.map((tech, index) => (
              <li className="mt-4 mb-12 mx-4 hover:scale-150 transition-transform duration-300" key={index}>
                <a href={tech.url} target="_blank" rel="noreferrer">
                  <img src={tech.src} alt={tech.alt} className="w-6 h-auto" loading="lazy" />
                </a>
              </li>
            ))}
          </ul>
