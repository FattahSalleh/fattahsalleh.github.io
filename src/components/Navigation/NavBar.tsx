...
export default function NavBar({ isOverlayOpen, setIsOverlayOpen }: NavBarProps) {
    ...
    <>
      ...
      <nav
        className={`lg:max-h-24 w-full lg:flex flex-no-wrap fixed top-0 z-10 flex-row justify-between px-12 py-4 items-center transition-all duration-500 
			hidden 
            ${isScrolled ? 'bg-darkBg text-secondary opacity-95' : 'text-lightText'}`}
        id="navBar"
      >
        <div>
          <a href="/">
      ...
      <div
        ...
        className={`fixed top-0 right-0  w-8/12 h-full bg-black bg-opacity-90 bg-darkGrayBg flex flex-col justify-center items-center z-20 transition-all duration-500 ${
          isOverlayOpen ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-full'
        }`}
      >
        <button className="absolute top-6 right-6 text-lightText" onClick={toggleOverlay}>
          <img src={closeIcon} alt="Close Icon" width={36} height={36} loading="lazy" />
        </button>
