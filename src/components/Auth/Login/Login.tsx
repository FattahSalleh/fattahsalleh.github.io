...
export default function Login() {
  ...
  const handleLogin = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!isSigningIn) {
        setIsSigningIn(true);
        await doSignInWithEmailAndPassword(sanitizedEmail, sanitizedPassword);
      }
    } catch (error) {
      console.error('Error during login:', error);
      setIsSigningIn(false);
      setError("Invalid email address or password. Please make sure you've entered the correct credentials.");
    }
  };
